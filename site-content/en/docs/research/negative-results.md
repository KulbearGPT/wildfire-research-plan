...
- **Measured negative/mixed:** a valid evaluated comparison fails its primary, attribution, clean-input, transfer or compression criterion. Positive cells remain in the positive collection.
- **Invalid/confounded historical foundation:** faulty pooled-year exposure or inherited initialization prevents a corrected-data efficacy claim; the raw observation is preserved.
- **Cancelled/unrun:** no method efficacy conclusion is allowed. Includes X7 overlap cancellation, X21/X24 idea rejection, D3 identifiability rejection, cancelled repairs and never-released confirmations.
- **Preparation/runtime failure or pending/unreported:** failed data alignment, OOM, dependency failure and missing final results are not negative model results.

## Full cross-history register, including mixed positives
...
| X2 — X2 feature distillation | fresh continuation; yes; original and block-local rejected; original rejected; block-local stopped | docs/experiments/t1_t5_innovations.md |
...
| X5 — X5 localized consistency | unchanged global D1 consistency; incremental method contribution; -.002294; reject; cancelled after T1 failure | docs/experiments/t1_t5_innovations.md |
...
| X7 — X7 missingness-conditioned residual experts | fresh continuation; no; overlaps archived D7-CRA; cancelled after overlap audit; cancelled after overlap audit | docs/experiments/t1_t5_innovations.md |
...
| X9 — X9 spatial-impact FiLM | fresh continuation with spatial route; yes; context-conditioned decoder modulation; routed -.000552; reject; cancelled after T1 failure | docs/experiments/t1_t5_innovations.md |
...
| X11 — X11 identifiable dynamic restoration | X10 forecast-only inpainting; incremental reconstruction objective; -.004862 vs X10; reject; cancelled after T1 failure | docs/experiments/t1_t5_innovations.md |
...
| X13 — X13 normalized diffusion inpainting | same frozen control checkpoint with spatial route; yes; parameter-free typed spatial propagation; routed -.024799; reject; cancelled after T1 failure | docs/experiments/t1_t5_innovations.md |
...
| X15 — X15 distance-to-evidence prompting | fresh continuation with spatial route; yes; continuous missing-geometry encoder prompt; routed -.002072; reject; cancelled after T1 failure | docs/experiments/t1_t5_innovations.md |
| X16 — X16 block-specialized dynamic restoration | X14 specialist plus fresh-control total check; no; restoration loses to X14; -.000452 vs X14; reject; cancelled after T1 attribution failure | docs/experiments/t1_t5_innovations.md |
| X17 — X17 severity-factorized block specialists | X14 mixed-severity specialist plus fresh ERM; no; secondary only after heldout attribution failure; ERM-positive all years; X14 2023 block -.000649; ERM-positive all years; X14 2022/23 block -.000898/-.001614 | docs/experiments/t1_t5_innovations.md |
...
| X20 — X20 ERM-anchored impact consistency | fresh ERM; X8 diagnoses the repair; same impact-consistency family as X8; -.003678; reject; cancelled after T1 failure | docs/experiments/t1_t5_innovations.md |
| X21 — X21 first-layer reliability calibration | D4/D12 and fresh ERM; no; pre-run closest-work reject; not run; not run | docs/experiments/t1_t5_innovations.md |
...
| X24 — X24 frozen-clean FireDrop distillation | X8 impact consistency and plain FireDrop specialist; no; closest-work reject before implementation; not run; not run | docs/experiments/t1_t5_innovations.md |
...
| X28 — X28 VIIRS reliability-footprint augmentation | cosine rectangular BlockDrop specialist; X22 cosine ERM adoption; no; geometry transfer fails decisively; -.034394 vs rectangle, -.027177 vs X22; reject; -.036784 vs rectangle, -.029450 vs X22; reject | docs/experiments/t1_t5_innovations.md |
...
| X30 — X30 training-only fire-change auxiliary heads | X22 cosine ERM; no; main-task interference in both architectures; -.010378 vs X22; reject; -.010875 vs X22; reject | docs/experiments/t1_t5_innovations.md |
| X31 — X31 multi-scale forecast deep supervision | X22 cosine ERM; no; same-target auxiliary interference; -.007688 vs X22; reject; -.015340 vs X22; reject | docs/experiments/t1_t5_innovations.md |
...
| X3-decoupled — Decoupled transition repair | T1 primary -0.045065 versus fresh control; T5 repair cancelled. | docs/experiments/t1_t5_innovations.md |
| X4-strong — Spatial risk 5x repair | T1/T5 cancelled after 11 seconds before meaningful computation; no efficacy result. | docs/experiments/t1_t5_innovations.md |
| X2-block-repair — Block-local feature KD | T1 primary -0.013556/block -0.001936; T5 stopped. | docs/experiments/t1_t5_innovations.md |
...
The full pre-cleanup tree is commit `4b843add48862dcfac668b643ecd2a564cbe00df`. P03 routes legacy P00/P02; P04–P06 test output/decoder correction, P07 stochastic residuals, P08 a clean posterior teacher, P09 year-corruption GroupDRO, P10 matched ERM, P11 a missingness router, P12 normalized focal alpha and P13 a FireDrop expert. D2 normalizes convolution by reliability; D4 learns an input token; D5 weights consistency by counterfactual impact; D6 adds ranking; D7/D8 use reliability/factorized adapters; D9 ranks error pairs; D10/D11 use deep/input prompt pyramids; D13 transfers SARP to T5. Exact implementation details and fixed settings are retained in the archived `docs/experiments/quantitative_reliability_ledger.md` and `p00_p06_rapid_reliability.md`.

| ID / implementation | Observation and nearest comparator | Source |
|---|---|---|
| legacy-c00-c02-3k — Legacy C00/C02 3K | 2021 AP 0.547372/0.584540; pooled-year resolver was invalid; runtime evidence only | docs/experiments/rejected_experiments.md |
| legacy-c00-c02-six-10k-runs — Legacy C00/C02 six 10K runs | completed and used for 96 M00--M07 evaluations; training exposure invalid; retain only the failure-regime diagnosis | docs/experiments/rejected_experiments.md |
...
| P09 — P09 GroupDRO | 2021 M06/M07 0.365531/0.185136; tied matched P10 ERM; no GroupDRO attribution | docs/experiments/rejected_experiments.md |
...
| P04 — P04 1×1 residual | M06/M07 0.332083/0.139913, below P03 0.350878/0.166982; insufficient correction | docs/experiments/rejected_experiments.md |
| P05 — P05 3×3 residual | M06/M07 0.327723/0.138767; extra output capacity did not help | docs/experiments/rejected_experiments.md |
| P06 — P06 decoder adaptation | M06/M07 0.330240/0.141035; below P03 | docs/experiments/rejected_experiments.md |
| P07 — P07 stochastic residual | predictive variance approximately 1e-8; latent state not identifiable | docs/experiments/rejected_experiments.md |
| P08 — P08 teacher posterior | M06/M07 0.306096/0.123222; below P00 | docs/experiments/rejected_experiments.md |
| P11 — P11 missingness router | M01 0.291202 versus P00 0.299465; routed complete FireDrop regressed | docs/experiments/rejected_experiments.md |
| P12 — P12 focal alpha | M06/M07 0.070432/0.061832 with severe overprediction; prevalence-derived alpha rejected | docs/experiments/rejected_experiments.md |
...
| belief-filter-t-1 — belief filter T=1 | M01/M06/M07 0.260952/0.316738/0.131167; mean delta -0.012264 versus P00; reject | docs/experiments/rejected_experiments.md |
| belief-attention-t-1 — belief attention T=1 | M01/M06/M07 0.274733/0.316787/0.130474; mean delta -0.007884; reject | docs/experiments/rejected_experiments.md |
...
| D3 — D3 temporal fusion | no GPU run; frozen corruptions cannot identify temporal selection; — | docs/experiments/rejected_experiments.md |
...
| D9 — D9 error-pair rank | 0.528919/0.260790/0.325537/0.153634; primary -0.016265; M00 -0.054101; 21117395 | docs/experiments/rejected_experiments.md |
...
## Distinct later normalization, weight-merge, branch and KD experiments
...
| RF-BN-fixed — Original fixed-forward BN diagnostic | Conditional minus original X22 primary -.260871/-.096705; unstable layer activations; superseded by batch-stat diagnostic. | docs/experiments/three_directions_results.md |
...
`codex/belief-filter` (`fac140f`), `codex/belief-attention` (`f451934`) and `codex/belief-reconstruction` (`12cdc2d`) have August 27, 2026 tips; they are old historical probes, not new September directions. The archive contains `state_filter.py`, `state_attention.py`, `reconstruction_baseline.py`, `train_belief_state.py` and `evaluate_belief_state.py`. All six T1/T5 primary comparisons were negative; numeric T1 values are above. T5 magnitudes are not stated in the compact rejection summary and must not be invented. Natural VIIRS attention’s +.000287 AP and target-QA’s .382293→.398789 AP are diagnostic positives with explicit metric/population caveats, not successful forecasting-method claims. The added-year QA alignment preparation failed on missing CRS/geotransform; it is not an efficacy experiment.

## Unfinished architecture matrix
...
## Artifact and source retention boundary

Source refs and recipe pointers are machine-readable in [method-inventory.json](method-inventory.json). Existing compact JSON result tables remain committed for both three-direction campaigns (legacy hyphenated files require branch recovery). Larger run summaries/checkpoints/logs remain external. Historical narrative without a compact JSON is evidence of the reported result, not a substitute for recovered raw evaluation artifacts. Never label archived code portable or reproduced solely because Git can recover it.
