...
if [[ $# -ne 3 ]]; then
  echo "usage: $0 P00_COMPLETED_RECORD P02_COMPLETED_RECORD P09_CHECKPOINT" >&2
  exit 2
fi
...
run_root=${base}/runs/prototype-P10-YearBalancedERM-${SLURM_JOB_ID}
erm_checkpoint=${run_root}/erm.pt
...
cp "${runner_path}" "${run_root}/run_environment_erm_on_nibi.sh"
...
sha256sum "${p00_record}" "${p02_record}" "${p09_checkpoint}" "${stats}" > "${run_root}/inputs.sha256"
