...
run_root=${base}/runs/prototype-P01-FireDropMask-C00-${SLURM_JOB_ID}
...
cp "${runner_path}" "${run_root}/run_reliability_prototype_on_nibi.sh"
...
printf '%q ' python -m reproductions.wsts_fast_track.reliability_prototype_entrypoint \
...
python -m reproductions.wsts_fast_track.reliability_prototype_entrypoint \
  --upstream-root "${upstream}" \
  --data-root "${data}" \
  --run-root "${run_root}" \
  --stats-path "${stats}" 2>&1 | tee "${run_root}/training.log"

python -m reproductions.wsts_fast_track.reliability_prototype_completion \
  --run-root "${run_root}" \
