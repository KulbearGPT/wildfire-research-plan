...
if [[ $# -ne 2 ]]; then
  echo "usage: $0 P00_COMPLETED_RECORD YEAR" >&2
  exit 2
fi
...
case "${year}" in
  2021|2022|2023) ;;
  *)
    echo "year must be 2021, 2022, or 2023" >&2
    exit 2
    ;;
esac
...
run_root=${base}/runs/prototype-P00-eval-${year}-${SLURM_JOB_ID}
...
cp "${runner_path}" "${run_root}/run_prototype_evaluation_on_nibi.sh"
