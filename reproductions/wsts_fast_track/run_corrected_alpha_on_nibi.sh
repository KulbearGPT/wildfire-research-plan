...
if [[ $# -ne 4 ]]; then
  echo "usage: $0 P00_COMPLETED_RECORD P02_COMPLETED_RECORD P09_CHECKPOINT P10_CHECKPOINT" >&2
  exit 2
fi
...
run_root=${base}/runs/prototype-P12-CorrectedFocalAlpha-${SLURM_JOB_ID}
p12_checkpoint=${run_root}/corrected-alpha.pt
...
cp "${runner_path}" "${run_root}/run_corrected_alpha_on_nibi.sh"
...
sha256sum "${p00_record}" "${p02_record}" "${p09_checkpoint}" "${p10_checkpoint}" "${stats}" > "${run_root}/inputs.sha256"
...
python -m reproductions.wsts_fast_track.evaluate_environment_dro \
  ...
  --corrected-alpha-checkpoint "${p12_checkpoint}" \
  --reliability-router \
  --output-root "${run_root}/results-2021" \
  --upstream-root "${upstream}" \
  --data-root "${data}" \
  --stats-path "${stats}" \
  --year 2021 \
...
test -f "${run_root}/results-2021/summary.json"
