...
variant=$2
case "${variant}" in
  erm) lambda_consistency=0.0 ;;
  kl) lambda_consistency=0.1 ;;
  *)
    echo "variant must be erm or kl" >&2
    exit 2
    ;;
esac
...
run_root=${base}/runs/D1-${variant}-S0-3K-${SLURM_JOB_ID}
checkpoint=${run_root}/D1-${variant}.pt
...
cp "${runner_path}" "${run_root}/run_predictive_consistency_on_nibi.sh"
...
python -m reproductions.wsts_fast_track.train_predictive_consistency \
  --b3-record "${b3_record}" \
...
python -m reproductions.wsts_fast_track.evaluate_predictive_consistency \
