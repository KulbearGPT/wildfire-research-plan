"""Evaluate the matched D2 standard-convolution control."""
...
def validate_d2_checkpoint(payload: Mapping[str, object]) -> str:
    """Enforce the exact matched D2-STD checkpoint contract."""

    valid = (
        payload.get("schema_version") == 1
        and payload.get("status") == "pass"
        and payload.get("candidate_id") == "D2-STD"
        and payload.get("matched_pair") == "D2"
        and payload.get("variant") == "standard"
    ...
    if not valid:
        raise ValueError("D2-STD checkpoint contract is invalid")
    return "D2-STD"
...
def load_d2_model(
    ...
    *,
    ...
    """Reconstruct the standard C00 model from a D2-STD payload."""
    ...
    return D2EvaluationModel(model).to(device)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    candidate_id = validate_d2_checkpoint(payload)
