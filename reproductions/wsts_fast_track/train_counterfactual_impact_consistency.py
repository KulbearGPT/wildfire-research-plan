"""Run the fixed D5-CIWC continuation from corrected B3."""
...
def ciwc_training_objective(
    model: Any,
    clean_logits: torch.Tensor,
    corrupt_logits: torch.Tensor,
    target: torch.Tensor,
) -> tuple[torch.Tensor, dict[str, float]]:
    """Return matched supervision plus the fixed CIWC term."""
    ...
    consistency = counterfactual_impact_kl_from_logits(
        clean_logits, corrupt_logits
    )
    objective = supervised + CIWC_WEIGHT * consistency
    return objective, {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for step in range(1, TRAINING_STEPS + 1):
        ...
        loss, final_parts = ciwc_training_objective(
        ...
        if step == 1 or step % 100 == 0:
            print(
                f"D5_STEP={step} LOSS={final_loss:.8f} "
                f"SUPERVISED={final_parts['supervised']:.8f} "
                f"CIWC={final_parts['consistency']:.8f}",
                flush=True,
            )
    ...
    print(f"D5_CHECKPOINT={output}", flush=True)
    return 0
