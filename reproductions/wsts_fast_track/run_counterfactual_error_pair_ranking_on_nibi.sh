...
run_root=${base}/runs/D9-CEPR-S0-3K-${SLURM_JOB_ID}
checkpoint=${run_root}/D9-CEPR.pt
...
cp "${runner_path}" "${run_root}/run_counterfactual_error_pair_ranking_on_nibi.sh"
...
python -m reproductions.wsts_fast_track.train_counterfactual_error_pair_ranking \
  --b3-record "${b3_record}" \
...
python -m reproductions.wsts_fast_track.evaluate_counterfactual_error_pair_ranking \
