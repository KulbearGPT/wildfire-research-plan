...
if [[ $# -ne 1 ]]; then
  echo "usage: $0 B3_COMPLETED_RECORD" >&2
  exit 2
fi
...
run_root=${base}/runs/D7-CRA-S0-3K-${SLURM_JOB_ID}
checkpoint=${run_root}/D7-CRA.pt
...
cp "${runner_path}" "${run_root}/run_counterfactual_reliability_adapter_on_nibi.sh"
...
python -m reproductions.wsts_fast_track.train_counterfactual_reliability_adapter \
  --b3-record "${b3_record}" \
...
python -m reproductions.wsts_fast_track.evaluate_counterfactual_reliability_adapter \
