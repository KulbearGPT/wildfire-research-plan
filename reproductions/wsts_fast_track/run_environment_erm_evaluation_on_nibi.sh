...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 P00_COMPLETED_RECORD P02_COMPLETED_RECORD P09_CHECKPOINT P10_CHECKPOINT YEAR" >&2
  exit 2
fi
...
erm_checkpoint=$(realpath "$4")
year=$5
case "${year}" in
  2022|2023) ;;
  *) echo "fixed P10 evaluation year must be 2022 or 2023" >&2; exit 2 ;;
esac
...
run_root=${base}/runs/prototype-P10-ERM-eval-${year}-${SLURM_JOB_ID}

test -f "${p00_record}"
test -f "${p02_record}"
test -f "${p09_checkpoint}"
test -f "${erm_checkpoint}"
test -f "${stats}"
if [[ -e "${run_root}" ]]; then
...
cp "${runner_path}" "${run_root}/run_environment_erm_evaluation_on_nibi.sh"
...
sha256sum "${p00_record}" "${p02_record}" "${p09_checkpoint}" "${erm_checkpoint}" "${stats}" > "${run_root}/inputs.sha256"
