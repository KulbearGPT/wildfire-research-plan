"""Evaluate the fixed D5-CIWC checkpoint on reliability scenarios."""
...
def validate_ciwc_checkpoint(payload: Mapping[str, object]) -> str:
    """Require the exact fixed D5-CIWC checkpoint contract."""

    valid = (
        payload.get("schema_version") == 1
        and payload.get("status") == "pass"
        and payload.get("candidate_id") == "D5-CIWC"
        and payload.get("matched_pair") == "D5"
        and payload.get("base_control") == "D1-ERM"
        and payload.get("closest_ablation") == "D1-KL"
    ...
    if not valid:
        raise ValueError("CIWC checkpoint contract is invalid")
    return "D5-CIWC"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("CIWC checkpoint must be a mapping")
    candidate_id = validate_ciwc_checkpoint(payload)
