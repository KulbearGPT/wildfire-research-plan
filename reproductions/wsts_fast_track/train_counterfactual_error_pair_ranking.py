"""Run the fixed D9-CEPR continuation from corrected B3."""
...
from .counterfactual_error_pair_ranking import (
    PAIR_FORMULATION,
    PAIR_TOP_K,
...
def cepr_training_objective(
    ...
    """Return fixed D5 objective plus counterfactual error-pair ranking."""
    ...
    pair = counterfactual_error_pair_ranking_from_logits(
        clean_logits,
        corrupt_logits,
        target,
        corrupted_sample,
    ...
    objective = supervised + CIWC_WEIGHT * ciwc + PAIR_WEIGHT * pair
    return objective, {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for step in range(1, TRAINING_STEPS + 1):
        ...
        corrupted_sample = (
            (fire_missing.flatten(start_dim=1).amax(dim=1) > 0)
            | (block_missing.flatten(start_dim=1).amax(dim=1) > 0)
        )
        optimizer.zero_grad(set_to_none=True)
        clean_logits = model(clean_x).squeeze(1)
        corrupt_logits = model(corrupt_x).squeeze(1)
        loss, final_parts = cepr_training_objective(
            model,
            clean_logits,
            corrupt_logits,
            target,
    ...
    payload = {
        ...
        "pair_formulation": PAIR_FORMULATION,
    ...
    print(f"D9_CHECKPOINT={output}", flush=True)
    return 0
