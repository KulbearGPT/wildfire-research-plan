...
run_root=${base}/runs/D11-CRPP-S0-3K-${SLURM_JOB_ID}
checkpoint=${run_root}/D11-CRPP.pt
...
cp "${runner_path}" "${run_root}/run_complete_reliability_prompt_pyramid_on_nibi.sh"
...
python -m reproductions.wsts_fast_track.train_reliability_prompt_pyramid \
  --b3-record "${b3_record}" \
...
python -m reproductions.wsts_fast_track.evaluate_reliability_prompt_pyramid \
