...
if [[ $# -ne 1 ]]; then
  echo "usage: $0 RELIABILITY_ROOT" >&2
  exit 2
fi

reliability_root=$(realpath "$1")
...
run_root=${base}/runs/viirs-reliability-screen-${SLURM_JOB_ID}
output=${run_root}/result.json

test -f "${reliability_root}/manifest.json"
test -f "${p00_record}"
...
cp "${runner_path}" "${run_root}/run_viirs_reliability_screen_on_nibi.sh"
...
printf '%q ' python -m reproductions.wsts_fast_track.evaluate_viirs_reliability \
  --p00-record "${p00_record}" \
  --attention-checkpoint "${attention_checkpoint}" \
  --reliability-root "${reliability_root}" \
  --output "${output}" \
...
python -m reproductions.wsts_fast_track.evaluate_viirs_reliability \
  --p00-record "${p00_record}" \
  --attention-checkpoint "${attention_checkpoint}" \
  --reliability-root "${reliability_root}" \
  --output "${output}" \
