...
run_root=${base}/runs/D5-CIWC-S0-3K-${SLURM_JOB_ID}
checkpoint=${run_root}/D5-CIWC.pt
...
cp "${runner_path}" "${run_root}/run_counterfactual_impact_consistency_on_nibi.sh"
...
python -m reproductions.wsts_fast_track.train_counterfactual_impact_consistency \
  --b3-record "${b3_record}" \
...
python -m reproductions.wsts_fast_track.evaluate_counterfactual_impact_consistency \
