...
Choose a new output path every time. Available cases are `rnc`, `token`, `ciwc`, `rank`, `cra`, `ffca`, `pyramid`, `complete-pyramid`, `sarp-t1`, `standard-t5`, `sarp-t5`, `B1`, `B5`. Use separate jobs for bounded failures and resource accounting. `rnc` verifies the combined D2/D4 implementation dependency; it does not designate RNC as a positive result.
