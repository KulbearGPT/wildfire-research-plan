...
Required environment: `WILDFIRE_ROOT`, `WILDFIRE_UPSTREAM`, `WILDFIRE_DATA`, `WILDFIRE_STATS`; the upstream checkout must have the documented import patch. Continuation cases additionally require valid original or regenerated completion records at `$WILDFIRE_ROOT/checkpoints/B3-completed.json` and/or `B5-completed.json`, with checkpoint paths resolving to the transferred files. B1/B2/B3/B5 bootstrap cases require cached or downloadable ImageNet encoder weights but do not need existing wildfire checkpoints.
...
Choose a new output path every time. Available cases are `d1-kl`, `d1-paired0`, `d12-sarp`, `rnc`, `token`, `ciwc`, `rank`, `cra`, `ffca`, `pyramid`, `complete-pyramid`, `sarp-t1`, `standard-t5`, `sarp-t5`, `B1`, `B2`, `B3`, `B5`, `legacy-P00`. Use separate jobs for bounded failures and resource accounting. `rnc` verifies the combined D2/D4 implementation dependency; it does not designate RNC as a positive result.

`d1-kl` runs the original predictive-consistency trainer with lambda 0.1; `d1-paired0` runs its matched paired-supervision control with lambda 0.0. Both use the D1 metadata validator and C00 model loader. `d12-sarp` runs the original `train_severity_adaptive_reliability_prompting` entrypoint and its `validate_sarp_checkpoint`/`load_sarp_model` evaluator, including the routing-mask input. The existing `sarp-t1` case instead exercises the severity-adaptive variant of `reliability_prompt_pyramid`; it is not evidence that the original D12 entrypoint ran.
...
B1/B2/B3/B5 cases temporarily replace the baseline spec's `max_steps` with two and set batch2/workers0, validation interval one and one validation batch. B2/B3 exercise the T1 C00 upstream training path (fire/fire-block corruption respectively); B1/B5 use T5 C02. Reload and evaluation select that same history. The successful two-step receipt is relabeled `bootstrap-qualification.json` and removed from the normal completion-record location. Its max_steps remains two, so it cannot satisfy the formal 3,000-step completion contract. The enclosing `QUALIFICATION_ONLY.json` is written before any work, including for failed jobs. All these artifacts must remain in the qualification tree.
...
`qualify-attention.py --output "$WILDFIRE_ROOT/qualification/attention-$SLURM_JOB_ID"` separately exercises the actual `train_belief_state` attention/history1 training path from the checksum-verified transferred `diagnostics/p00-completed.json` and `diagnostics/artifact-map.json`. Run it under the same allocated GPU environment with `WILDFIRE_SOURCE_COMMIT` set to the archived source SHA (`job.sh` exports this automatically). No Git metadata is required inside the exported checkout. It temporarily changes only the trainer's update budget to one; physical batch2 and accumulation32 preserve effective batch64. This performs 32 real training microbatches and can take longer than the continuation smoke cases.

The attention checkpoint retains `steps: 1`, `status: qualification`, and explicit non-scientific markers. The unmodified formal evaluator must reject it. An in-memory validation copy substitutes status/pass and steps/3000 and removes only the two added qualification markers required by the evaluator's exact-key schema; all other metadata, provenance, finite-loss and tensor checks run unchanged. No adapted checkpoint is written. The helper uses the actual trainable-state loader, verifies tensor equality, evaluates two real 2021 samples each for M00/M06 against frozen P00, and checks exact M00 logits. This qualifies attention training/save/reload, not natural-VIIRS performance, full training convergence, or regeneration of the retained positive result.
