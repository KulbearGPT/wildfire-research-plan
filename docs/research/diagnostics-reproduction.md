...
Source AST parsing and relative-import closure checks passed. Five stdlib tests verify relocation, unchanged provenance, missing-map-entry rejection, independent byte counts, NPZ manifest integrity, path containment and corruption detection. The two original VIIRS mechanism test files are recovered for execution in Slurm, not run on login. Fresh-environment imports passed in CPU job `22102394`. GPU job `22102452` (source `e13b9fb5db55`, node `g36`) completed both actual 24-sample evaluations using relocated, checksum-verified artifacts. The [new replay results](environment/diagnostics-replay.json) remain separate from the historical results. Natural AP delta is +0.0002895645 (historical +0.0002865119); QA-censored AP delta is −0.0008735966 (historical −0.0008778928). The direction of both comparisons, Brier deterioration, and QA ranking reversal agree. Across corresponding numeric fields, the largest absolute difference is 0.00000429624; this is not bitwise numerical identity or a new efficacy experiment. This qualifies inference with transferred weights, not full-budget training or regenerated historical scores. Separately, GPU job `22103112` completed one actual attention optimizer update, strict trainable-state reload, formal short-checkpoint rejection and controlled M00/M06 evaluation; see the [final qualification batch](environment/gpu-qualification-final-batch.json).
