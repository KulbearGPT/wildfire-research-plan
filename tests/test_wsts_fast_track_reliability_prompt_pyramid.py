...
def test_rpp_runner_has_valid_shell_contract(tmp_path: Path) -> None:
    runner = (
        Path(__file__).resolve().parents[1]
        / "reproductions"
        / "wsts_fast_track"
        / "run_reliability_prompt_pyramid_on_nibi.sh"
    )
...
def test_generic_heldout_runner_accepts_rpp_before_year_validation(
    tmp_path: Path,
) -> None:
    runner = (
        Path(__file__).resolve().parents[1]
    ...
    checkpoint = tmp_path / "rpp.pt"
    checkpoint.write_bytes(b"placeholder")

    result = subprocess.run(
        [str(runner), "rpp", str(checkpoint), "2021", "D10-RPP"],
        text=True,
        capture_output=True,
        check=False,
    )
